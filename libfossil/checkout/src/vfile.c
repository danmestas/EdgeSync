...
int fsl_vfile_changes_scan(fsl_cx * const f, fsl_id_t vid, unsigned cksigFlags){
  ...
  fsl_size_t rootLen;
  const fsl_time_t currentSysTime = (fsl_time_t)time(0);
  fsl_buffer * const fileCksum = fsl__cx_scratchpad(f);
  ...
  if(vid<=0) vid = f->db.ckout.rid;
  assert(vid>=0);
  rootLen = fsl_strlen(f->db.ckout.dir);
  assert(rootLen);
...
int fsl__vfile_to_ckout(fsl_cx * const f, fsl_id_t vfileId,
  ...
  assert(qArg>=0);
  rc = fsl_db_prepare(db, &q, sql, f->db.ckout.dir, qArg, __func__);
  if(rc){
  ...
  while(FSL_RC_STEP_ROW==(rc = fsl_stmt_step(&q))){
    ...
    char const * zRelName = &zName[f->db.ckout.dirLen];
    int isMod = 0;
    ++counter;
    assert(nameLen > f->db.ckout.dirLen);
