...
*/
bool fsl_islower(int c){ return c>='a' && c<='z'; }
bool fsl_isupper(int c){ return c>='A' && c<='Z'; }
bool fsl_isdigit(int c){ return c>='0' && c<='9'; }
...
bool fsl_isalpha(int c){
  return (c>='a' && c<='z') || (c>='A' && c<='Z');
}
bool fsl_isalnum(int c){
  return (c>='a' && c<='z') || (c>='A' && c<='Z') || (c>='0' && c<='9');
}


void fsl_os_str_free(void *pOld){
#if defined(_WIN32)
  fsl_free(pOld);
  ...
  *qUtf = 0;
  ...
  zOut = fsl_strdup(zFilename)
    /*
      ...
      so that fsl_os_str_free() can DTRT.
    */;
  return zOut;
  ...
  */
  ...
  return fsl_strdup(zUtf8)
    /* Why? Why not just act like Unix?
       Much later: so that fsl_os_str_free() can DTRT. */;
#else
