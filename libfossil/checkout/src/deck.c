...
FSL__CARD_F_LIST_NEEDS_SORT = 0x01
};
...
/**
   ...
   returned without causing side effects beyond adjustment of caching
   metrics.
*/
...
/**
   ...
   and the found deck->type differs from the 4th argument (type) then
   false is returned, FSL_RC_TYPE is returned via *rc, and f's error
   state is updated with a description of the problem. In all other
   case *rc is set to 0.
*/
...
static fsl_card_F * fsl_card_F_list_push( fsl_card_F_list * li ){
  ...
  li->list[li->used] = fsl_card_F_empty;
  if(li->used){
    li->flags |= FSL__CARD_F_LIST_NEEDS_SORT/*pessimistic assumption*/;
  }
...
static void fsl_deck_clean_J(fsl_deck * const m){
  fsl__card_J_list_free(&m->J, true);
}
...
void fsl_deck_clean2(fsl_deck * const m, fsl_buffer * const xferBuf){
  ...
  CBUF(content)/* content must be after all cards because some point
                  back into it and we need this memory intact in order
                  to know that. */;
  {
...
#if 0
      /* Is this a valid check or overzealous? It's currently breaking
         clone testing. Possibly rightfully so, but that's as-yet
         undetermined. fossil(1) does not do this check. */
      else if(d->R
               && !d->F.used
...
#define C(X,V) case X: return &#X[11];
    fsl_satype_e_map(C)
#undef C

#define C(T1,T2) case T1: t=T2; goto again
    C(FSL_SATYPE_BRANCH_START, FSL_SATYPE_CHECKIN);
    //C(FSL_SATYPE_EVENT, FSL_SATYPE_TECHNOTE);
#undef C
...
int fsl_deck_K_set( fsl_deck * const mf, fsl_uuid_cstr uuid){
  if(uuid){
    ...
    unsigned char rnd[FSL_STRLEN_SHA1/2];
...
static void fsl_card_F_list_sort(fsl_card_F_list * li){
  if(FSL__CARD_F_LIST_NEEDS_SORT & li->flags){
    qsort(li->list, li->used, sizeof(fsl_card_F),
          fsl_card_F_cmp );
    li->flags &= ~FSL__CARD_F_LIST_NEEDS_SORT;
  }
}
...
int fsl_deck_F_add( fsl_deck * const mf, char const * name,
  ...
  if(!mf || !name) return FSL_RC_MISUSE;
    ...
    if(!t) return FSL_RC_OOM;
    assert(mf->F.used>1
           ? (FSL__CARD_F_LIST_NEEDS_SORT & mf->F.flags)
           : 1);
    assert(!t->name);
    assert(!t->uuid);
    assert(!t->priorName);
...
int fsl_deck_output( fsl_deck * const d, fsl_output_f out,
  ...
  if(os->rc && os->error.code){
    fsl_error_propagate(&os->error, &f->error);
  }
...
static fsl_card_F * fsl__deck_F_seek_base(fsl_deck * d,
  ...
  else if(FSL__CARD_F_LIST_NEEDS_SORT & d->F.flags){
...
int fsl_deck_F_set( fsl_deck * d, char const * zName,
  ...
  if(fc){
    ...
    if(priorName && *priorName){
      if(0==fsl_cx_filename_cmp(d->f, priorName, fc->name)){
        return fsl_cx_err_set(d->f, FSL_RC_MISUSE,
                              "An F-card's priorName must not be the same "
                              "as its original name: %s", priorName);
      }else if(!fsl_is_simple_pathname(priorName, 1)){
        return fsl_cx_err_set(d->f, FSL_RC_RANGE,
...
void fsl__deck_clean_cards(fsl_deck * const d, char const * letters){
  ...
  for( ; *c; ++c ){
    switch(*c){
      ...
      case 'J': fsl_deck_clean_J(d); break;
...
int fsl_deck_derive(fsl_deck * const d){
  ...
  while(d->B.uuid){
    /* This is a delta manifest. Convert this deck into a baseline by
       building a new, complete F-card list. */
    fsl_card_F const * fc;
...
static int fsl_deck_xlink_f_control(fsl_deck * const d,
  ...
  /*
    ...
    fossil(1). This crosslinker can, however, be replaced by users
    concerned by that.
  */
  int rc = 0;
  fsl_buffer * const comment = fsl__cx_scratchpad(d->f);
...
int fsl__deck_crosslink_one( fsl_deck * const d ){
  ...
  assert(0!=fsl_db_txn_level(fsl_cx_db_repo(d->f))
...
int fsl__deck_crosslink( fsl_deck /* const */ * const d ){
  ...
  if(!rc){
    rc = fsl_db_txn_end(db, false);
  }else{
    if(db->error.code && !f->error.code){
      fsl_cx_uplift_db_error(f,db);
    }
    fsl_db_txn_end(db, true);
  }
...
/**
    ...
    a legal string.
 */
static bool fsl__is_after_blank_line(const char *z){
  if( z[-1]!='\n' ) return false;
  if( z[-2]=='\n' ) return true;
  if( z[-2]=='\r' && z[-3]=='\n' ) return true;
  return false;
...
/** @internal
    ...
    Remove the PGP/SSH signature from a raw artifact, if there is one.

    Expects *pz to point to *pn bytes of string memory which might or
    might not be prefixed by a PGP/SSH signature.  If the string is
    enveloped in a signature, then upon returning *pz will point to
    the first byte after the end of the PGP/SSH header and *pn will
    contain the length of the content up to, but not including, the
    PGP footer.

    If *pz does not look like a PGP/SSH header then this is a no-op.
...
static void fsl__remove_pgp_signature(unsigned char const **pz, fsl_size_t *pn){
  ...
  fsl_size_t n = (fsl_size_t)*pn;
  fsl_size_t i;
  char const * const aMsg[2] = {
    // must all be 34 bytes
    "-----BEGIN PGP SIGNED MESSAGE-----",
    "-----BEGIN SSH SIGNED MESSAGE-----"
  };
  char const * const aSig[2] = {
    "\n-----BEGIN PGP SIGNATURE-",
    "\n-----BEGIN SSH SIGNATURE-"
  };
  if( n<59 ) return;
  for(int m = 0; m < 2; ++m ){
    if( 0==memcmp(z, aMsg[m], 34) ){
      for(i=34; i<n && !fsl__is_after_blank_line((char const *)(z+i)); i++){}
      if( i>=n ) continue;
      z += i;
      *pz = z;
      unsigned char const * bps =
        (unsigned char const *)strstr((char const *)z, aSig[m]);
      if( bps ){
...
unsigned char * fsl__tokenizer_next(fsl__tokenizer * const p,
                                    fsl_size_t * const pLen){
  if( p->atEol ) return NULL;
  unsigned char * const zStart = p->z;
#if 1
  /* HOLY COW, this impl requires some 1/3rd the CPU instructions of
     the one in the #else block, despite ostensibly doing the same
     work! */
  size_t const n = strcspn((const char *)p->z, (char const *)" \n");
  p->atEol = p->z[n]=='\n';
  p->z[n] = 0;
  p->z += n+1;
  *pLen = n;
 ...
 */
static unsigned char deck__next_card(fsl__tokenizer * const p){
  unsigned char c;
...
static fsl_satype_e fsl_deck_guess_type( const int * l ){
  ...
  /*
     ...
     identify any syntactically-legal combination of cards.

     Note that the order of these checks is sometimes significant,
     which is why we don't put them in expected order of frequency
     (i.e. checkins first).
  */
#define L(X) l[X-'A']
#define CHECK(T,COND) if(COND) return T
  CHECK(FSL_SATYPE_CLUSTER,    L('M'));
  CHECK(FSL_SATYPE_EVENT,      L('E'));
  CHECK(FSL_SATYPE_FORUMPOST,  L('G') || L('H') || L('I'));
  CHECK(FSL_SATYPE_WIKI,       L('L') || L('W'));
  CHECK(FSL_SATYPE_TICKET,     L('J') || L('K'));
  CHECK(FSL_SATYPE_ATTACHMENT, L('A'));
  CHECK(FSL_SATYPE_CHECKIN,
        L('F') || L('C') || L('P') ||
        L('Q') || L('R') || L('B'));
  CHECK(FSL_SATYPE_CONTROL,    L('D') && L('T') && L('U'));
#undef CHECK
#undef L
...
bool fsl_might_be_artifact(fsl_buffer const * const src){
  ...
  if(n<36) return false;
  else if(z[0]<'A' || z[0]>'M' || z[1]!=' '
          /*          ^^^^^^^ we "could" check for 'D' instead, but
                      cluster artifacts are the exception to the
                      D-card requirement rule. */
          || z[n-35]!='Z'
          || z[n-34]!=' '
...
#define SYNTAX(MSG) ERROR(rc ? rc : FSL_RC_SYNTAX,MSG)
  ...
  int rc = 0;
  fsl__tokenizer x = fsl__tokenizer_empty;
  ...
  */
  ...
  if(rid>0 && (fsl__cx_mcache_search2(f, rid, d, FSL_SATYPE_ANY, &rc)
    ...
    if( n ){
      return fsl_error_set(err, FSL_RC_SYNTAX,
      //"Deck is not terminated with \\n: %b", src);
                           "Deck is not terminated with \\n");
    }else{
      return fsl_error_set(err, FSL_RC_SYNTAX, "Deck has no content");
    }
  }
  ...
  */
  ...
  /* Parsing helpers... */
#define TOKEN(DEFOS) tokLen=0; token = fsl__tokenizer_next(&x,&tokLen); \
    ...
    switch(cType){
      ...
      */
      ...
      /*
         ...
         for Wiki pages, Events, Attachments, and are disallowed on
         all other control files.
      */
      case 'C':{
        if( d->C ){
      ...
      /*
         ...
         The timestamp should be ISO 8601 YYYY-MM-DDtHH:MM:SS[.sss].
         There can be no more than 1 D line.  D lines are required for
         all control files except for clusters.
      */
      case 'D':{
#define TOKEN_DATETIME(LETTER,MEMBER)                                     \
        ...
        if(!fsl_iso8601_to_julian((char const *)token, &ts)){   \
          SYNTAX("Malformed date in "#LETTER"-card"); \
        } (void)0
      ...
      /*
         ...
         event in the format YYYY-MM-DDtHH:MM:SS[.sss] and a unique
      ...
      */
      case 'F':{
        ...
        ++stealBuf;
        assert(d->F.used>1
               ? (FSL__CARD_F_LIST_NEEDS_SORT & d->F.flags)
               : 1);
  ...
  if(stealBuf>0){
    /* We stashed something which points to src->mem, so we need to
       steal that memory. */
    fsl_buffer_swap_free( &d->content, src, 1 );
  }else{
  ...
  d->rid = rid;
  d->F.flags &= ~FSL__CARD_F_LIST_NEEDS_SORT/*we know all cards were read in order*/;
  return 0;
...
static int fsl__deck_baseline_load( fsl_deck * d ){
  int rc = 0;
  fsl_deck bl = fsl_deck_empty;
  fsl_id_t rid;
  fsl_cx * const f = d ? d->f : NULL;
  ...
  if(rid<0){
    ...
    if(d->rid>0){
      fsl_cx_exec(f,
                  "INSERT OR IGNORE INTO %!Q.orphan(rid, baseline) "
                  "VALUES(%"FSL_ID_T_PFMT",%"FSL_ID_T_PFMT")",
...
int fsl__deck_deltify_parent(fsl_deck * d){
  ...
  if(d->P.used){
    assert(FSL_SATYPE_CHECKIN == d->type
           || FSL_SATYPE_WIKI == d->type
           || FSL_SATYPE_FORUMPOST == d->type
           || FSL_SATYPE_EVENT == d->type);
    fsl_id_t const prid =
      fsl_uuid_to_rid(d->f, (char const *)d->P.list[0]);
    rc = prid>0
      ? fsl__content_deltify(d->f, prid, d->rid, 0)
      : d->f->error.code;
#if 0
    MARKER(("Deltifying parent(%d) of deck(%d) rc=%s...\n",
            (int)prid, (int)d->rid, fsl_rc_cstr(rc) ));
    fsl_db_each( fsl_cx_db(d->f), fsl_stmt_each_f_dump,
                 d->f, "SELECT CAST(fsl_uncompress(content) AS text) "
                 "FROM blob WHERE rid=%d",(int)prid);
#endif
...
int fsl_deck_save( fsl_deck * const d, bool isPrivate ){
  ...
  */
  ...
  /*
    ...
    2025-08-08: why have we not been doing this?
  */
  rc = fsl__deck_deltify_parent(d);
  if( rc ) goto end;
#endif
  ...
  switch(type){
    ...
    case FSL_SATYPE_TECHNOTE:
      rc = fsl_cx_prepare(f, &q,
                          ...
                          //"AND TYPEOF(x.value+0)='integer'"
                          // ^^^^ only 'event-%' tags which are technotes.
                          // UNTESTED on technotes (see WIKI, above) except
                          // in the SQLite shell, where it seems to behave.
                          "ORDER BY x.mtime DESC");
      break;
#if 0
    case FSL_SATYPE_ANY:
...
static const char * fsl__deck_type_json(fsl_satype_e t){
  switch(t){
    case FSL_SATYPE_CHECKIN: return "checkin";
    case FSL_SATYPE_CLUSTER: return "cluster";
    case FSL_SATYPE_CONTROL: return "tag"/*by request from drh*/;
    case FSL_SATYPE_WIKI: return "wiki";
    case FSL_SATYPE_TICKET: return "ticket";
    case FSL_SATYPE_ATTACHMENT: return "attachment";
    case FSL_SATYPE_TECHNOTE: return "technote";
    case FSL_SATYPE_FORUMPOST: return "forumpost";
    default:
      return NULL;
  }
}

int fsl_deck_to_json(fsl_deck const * d, fsl_output_f fn, void * fState){
  fsl_cx * const f = d->f;
  int rc = 0;
  char * z = 0;
  if( !f ) return FSL_RC_MISUSE;
  /** Maintenance reminder: this output is intended to remain
      compatible with fossil(1)'s. */
#define RC if(rc) goto end
#define APPN0(...) rc = fsl_appendn(fn, fState, __VA_ARGS__)
#define APPN(...) APPN0(__VA_ARGS__); RC
#define APPF0(...) rc = fsl_appendf(fn, fState, __VA_ARGS__)
#define APPF(...) APPF0(__VA_ARGS__); RC
  APPN("{", 1);
  if( d->rid ){
    z = fsl_rid_to_uuid(f, d->rid);
    if( !z ) return FSL_RC_OOM /* this assumes that d->rid is valid */;
    else if((rc = fsl_cx_err_get(f, NULL, NULL))){
      goto end;
    }
    APPF0("\"uuid\":%!j", z);
    fsl_free(z);
...
#define ISA(TYPE) if( d->type==TYPE )
#define CARD_LETTER(LETTER) \
  APPN(",\"" #LETTER "\":", -1)
  /*blob_appendf(b, ", \"rid\": %d", p->rid); not portable across repos*/
  APPF(", \"type\":%!j", fsl__deck_type_json(d->type));
#define CARD_STR(LETTER, VAL) \
  assert( VAL ); CARD_LETTER(LETTER); APPF("%!j", VAL)
#define CARD_STR2(LETTER, VAL) \
  if( VAL ) { CARD_STR(LETTER, VAL); } (void)0
#define STR_OR_NULL(VAL)                 \
  if( VAL ) { APPF("%!j", VAL); }        \
  else { APPN("null", 4); } (void)0
#define KVP_STR(ADDCOMMA, KEY,VAL)  \
  if(ADDCOMMA) {APPN(",",1);}       \
  APPF("%!j:", #KEY);              \
  STR_OR_NULL(VAL)

  ISA( FSL_SATYPE_ATTACHMENT ){
    CARD_LETTER(A);
    APPN("{",1);
    KVP_STR(0, filename, d->A.name);
    KVP_STR(1, target, d->A.tgt);
    KVP_STR(1, source, d->A.src);
    APPN("}",1);
  }
  CARD_STR2(B, d->B.baseline);
  CARD_STR2(C, d->C);
  CARD_LETTER(D); APPF("%f", d->D);
  ISA( FSL_SATYPE_TECHNOTE ){
    APPF(", \"E\":{\"time\":%f,\"id\":%!j}",
         d->E.julian, d->E.uuid);
  }
  ISA( FSL_SATYPE_CHECKIN ){
    CARD_LETTER(F);
    APPN("[",1);
    for( fsl_size_t i = 0; i < d->F.used; ++i ){
      fsl_card_F const * const cf = &d->F.list[i];
      char perms[] = {0,0,0};
      short iPerm = 0;
      if( i>0 ) {APPN(",",1);}
      APPN("{",1);
      KVP_STR(0, name, cf->name);
      KVP_STR(1, uuid, cf->uuid);
      if( cf->perm & FSL_FILE_PERM_EXE ) perms[iPerm++] = 'x';
      if( cf->perm & FSL_FILE_PERM_LINK ) perms[iPerm++] = 'l';
      KVP_STR(1, perm, iPerm ? &perms[0] : NULL);
      KVP_STR(1, rename, cf->priorName);
      APPN("}",1);
    }
  ...
  CARD_STR2(G, d->G);
  ISA( FSL_SATYPE_FORUMPOST ){
    CARD_LETTER(H);
    STR_OR_NULL((d->H && *d->H) ? d->H : NULL);
    CARD_STR2(I, d->I);
  }
  if( d->J.used ){
    CARD_LETTER(J);
    ...
    for( fsl_size_t i = 0; i < d->J.used; ++i ){
      fsl_card_J const * const cj = d->J.list[i];
      if( i>0 ) {APPN(",",1);}
      APPN("{",1);
      KVP_STR(0, name, cj->field);
      APPF(",\"append\":%s", cj->append ? "true" : "false");
      KVP_STR(1, value, cj->value);
      APPN("}",1);
    }
    APPN("]",1);
  }
  CARD_STR2(K, d->K);
  CARD_STR2(L, d->L);
  ISA( FSL_SATYPE_CLUSTER ){
    CARD_LETTER(M);
    APPN("[",1);
    for( fsl_size_t i = 0; i < d->M.used; ++i ){
      if( i>0 ) {APPN(",",1);}
      APPF("%!j", (const char *)d->M.list[i]);
    }
    APPN("]",1);
  }
  CARD_STR2(N, d->N);
  ISA( FSL_SATYPE_CHECKIN || d->P.used ){
    CARD_LETTER(P);
    APPN("[",1);
    for( fsl_size_t i = 0; i < d->P.used; ++i ){
      if( i>0 ) {APPN(",",1);}
      APPF("%!j", (const char *)d->P.list[i]);
    }
  ...
  if( d->Q.used ){
    CARD_LETTER(Q);
    APPN("[",1);
    for( fsl_size_t i = 0; i < d->Q.used; ++i ){
      fsl_card_Q const * const cq = (fsl_card_Q const *)d->Q.list[i];
      if( i>0 ) {APPN(",",1);}
      APPN("{",1);
      char type = '?';
      switch( cq->type ){
        case FSL_CHERRYPICK_ADD: type = '+'; break;
        case FSL_CHERRYPICK_BACKOUT: type = '-'; break;
        default: assert(!"Invalid Q-card"); break;
      }
      APPF("\"type\":\"%c\"", type);
      KVP_STR(1, target, cq->target);
      KVP_STR(1, base, cq->baseline);
      APPN("}",1);
    }
    APPN("]",1);
  }
  ...
  if( d->T.used ){
    CARD_LETTER(T);
    APPN("[",1);
    for( fsl_size_t i = 0; i < d->T.used; ++i ){
      fsl_card_T const * const c = (fsl_card_T const *)d->T.list[i];
      if( i>0 ) {APPN(",",1);}
      APPN("{",1);
      char type = '?';
      switch(c->type){
        case FSL_TAGTYPE_ADD: type = '+'; break;
        case FSL_TAGTYPE_CANCEL: type = '-'; break;
        case FSL_TAGTYPE_PROPAGATING: type = '*'; break;
        default: assert(!"Invalid tag type!"); break;
      }
      APPF("\"type\":\"%c\"", type);
      KVP_STR(1, name, c->name);
      KVP_STR(1, target, c->uuid ? c->uuid : "*");
      KVP_STR(1, value, c->value);
      APPN("}",1);
    }
    APPN("]",1);
  }
  CARD_STR2(U, d->U);
  ISA( FSL_SATYPE_WIKI || FSL_SATYPE_FORUMPOST==d->type
       || FSL_SATYPE_TECHNOTE==d->type || d->W.used ){
    CARD_LETTER(W);
    if( d->W.used ){ APPF("%!j", fsl_buffer_cstr(&d->W)); }
    else{ APPN("null", 4); }
  }
  APPN("}", 1);
end:
