    ...
    while( !rc && (FSL_RC_STEP_ROW == fsl_stmt_step(&s)) ){
      ...
      if(doit){
        ...
        if( FSL_TAGID_BGCOLOR == tagid ){
          ...
          rc = fsl__leaf_eventually_check(f, cid);
        }
      }
    }
    fsl_stmt_reset(&s);
...
int fsl__tag_insert( fsl_cx * const f, fsl_tagtype_e tagtype,
  ...
  if(FSL_TAGID_BRANCH == tagid ){
    rc = fsl__leaf_eventually_check(f, rid);
    if(rc) goto end;
  }
#if 0
...
int fsl_branch_create(fsl_cx * f, fsl_branch_opt const * opt, fsl_id_t * newRid ){
  ...
  else if(! (user = opt->user ? opt->user : f->db.repo.user)){
  ...
  */
  ...
  if(rc) goto end;
    ...
    else rc = fsl_cx_txn_end_v2(f, true, false);
    if(rc) goto end;
  }
#else
