...
/*
   The hash function
*/
#define fsl_id_bag_hash(i)  (((uint64_t)i)*101)
...
fsl_id_bag * fsl_id_bag_reuse(fsl_id_bag * const p){
...
int fsl_id_bag_copy(fsl_id_bag const * src, fsl_id_bag * dest){
  int rc = 0;
  for( fsl_id_t bid = fsl_id_bag_first(src);
       bid>0 && 0==rc;
       bid = fsl_id_bag_next(src, bid) ){
    rc = fsl_id_bag_insert(dest, bid);
  }
  return rc;
}

void fsl__id_bag_dump(fsl_id_bag const * bag, FILE *to,
                      char const *zHeader){
  if( !to ) to = stdout;
  fprintf(to, "%s:%d:%s() id bag size %d: %s\n",
          __FILE__, __LINE__, __func__, (int)bag->used, zHeader);
  int n = 0;
  for( fsl_id_t bid = fsl_id_bag_first(bag);
       bid>0;
       ++n, bid = fsl_id_bag_next(bag, bid) ){
    fprintf(to,"%d ", (int)bid);
  }
  if(n) fprintf(to,"\n");
}
