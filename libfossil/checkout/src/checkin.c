...
*/
static int fsl__checkin_import_file( fsl_cx * const f,
  ...
  rc = fsl_buffer_appendf(nbuf, "%s%s", f->db.ckout.dir, zRelName);
  nbuf->used = oldSize;
  if(rc) goto end;
...
*/
static int fsl__xqueue_callback(fsl_cx * f, fsl_stmt * const st,
                                fsl_id_t vfid,
  ...
  if(!st->stmt){
    rc = fsl_cx_prepare(f, st,
                        "SELECT pathname FROM %!Q.vfile "
                        "WHERE id=?1 /*%s()*/",
                        fsl_db_role_name(FSL_DBROLE_CKOUT),
                        __func__)
      /* TODO?: cache this. It's not called all that
         often. */;
    if(rc) return rc;
  }
  rc = fsl_stmt_bind_step(st, "R", vfid);
  switch(rc){
    case FSL_RC_STEP_ROW:{
...
int fsl_checkin_enqueue(fsl_cx * f, fsl_checkin_queue_opt const * opt){
  ...
  if(opt->vfileIds){
    if(!fsl_id_bag_count(opt->vfileIds)){
      rc = fsl_cx_err_set(f, FSL_RC_MISUSE,
                          ...
                          "may not be empty.")
        /* Is this too strict? Maybe just not enqueue anything? */;
      goto end;
    }
  }else{
  ...
  for(fsl_id_t vfid = fsl_id_bag_first(vfileIds);
    ...
    if(!rc
       ...
       /* Was enqueued */
       && opt->callback){
      rc = fsl__xqueue_callback(f, &qName, vfid, opt);
    }
  }
  end:
  if(opt->vfileIds){
  ...
  if(!rc && list.entryCount){
    ...
    for( fsl_id_t nid = fsl_id_bag_first(&list);
      ...
      if(fsl_id_bag_remove(&f->ckin.selectedIds, nid)
         && opt->callback){
        rc = fsl__xqueue_callback(f, &qName, nid, opt);
      }
    }
  }
  end:
...
*/
static
int fsl__checkin_calc_F_cards2( fsl_cx * f, fsl_deck * d,
...
#define RC if(rc) goto end
  ...
  while( FSL_RC_STEP_ROW==fsl_stmt_step(q) ){
    ...
    if(0 && changed){
      MARKER(("vfid=%d, isExe=%d, changed=%d\n", (int)id, isExe, changed));
    }
    if(isExe){
      if( FSL_VFILE_CHANGE_NOT_EXEC==changed ){
        perm = FSL_FILE_PERM_REGULAR;
      }else{
        perm = FSL_FILE_PERM_EXE;
      }
    }else if(isLink){
      ...
      */
      ...
      assert( !isExe );
      if( FSL_VFILE_CHANGE_IS_EXEC==changed ){
        perm = FSL_FILE_PERM_EXE;
      }else{
        assert( FSL_FILE_PERM_REGULAR==perm );
      }
    }
    ...
    while(pFile && fsl_strcmp(pFile->name, zName)<0){
      /* Baseline has files with lexically smaller names.
         ...
         that fsl_checkin_commit() thinks we have not made any changes
    ...
    if(rc) goto end;
      ...
      if(deleted){
        ...
        rc = fsl__checkin_import_file(f, zName, rid,
                                      ciOpt->allowMergeConflict,
                                      &fileBlobRid, &fUuid);
        if(!rc) rc = fsl__unsent_rid_add(f, fileBlobRid);
        RC;
...
/**
   ...
   basedOnVid must currently be f->db.ckout.rid OR the vfile table must
...
*/
static int fsl__checkin_calc_manifest( fsl_cx * f, fsl_deck * d,
                                      fsl_id_t basedOnVid,
...
#define RC if(rc) goto end
  ...
  { /* F-cards */
    ...
    rc = fsl__checkin_calc_F_cards2(f, d, pBase, basedOnVid,
                                   &szD, opt);
    /*MARKER(("szD=%d\n", (int)szD));*/
    RC;
    if(basedOnVid && !szD){
    ...
    */
    ...
    if(pBase && (deltaPolicy<0/*non-force-mode*/
      ...
      */
      ...
      szD = 0;
      rc = fsl__checkin_calc_F_cards2(f, d, NULL, basedOnVid,
                                     &szD, opt);
      RC;
      if(basedOnVid && !szD){
        rc = fsl_cx_err_set(f, errNoFilesRc, errNoFilesMsg);
...
int fsl_checkin_commit(fsl_cx * f, fsl_checkin_opt const * opt,
  ...
  fsl_id_t const vid = f ? f->db.ckout.rid : 0;
#if FSL_API_ARMOR
  if(!f || !opt) return FSL_RC_MISUSE;
  else
#endif
  if(!(dbC = fsl_needs_ckout(f))) return FSL_RC_NOT_A_CKOUT;
  else if(!(dbR = fsl_needs_repo(f))) return FSL_RC_NOT_A_REPO;
  assert(vid>=0);

  if( opt->dryRun ){
    MARKER(("WARNING: dry-run mode of checkin is not yet "
            "working due to a stale cache bug\n"));
  }
  ...
  */
  if( fsl_leaf_is_closed(f, vid)
    ...
    /* This is arguably being too strict, but it's normally the
       behavior we want. We could add a force flag to skip this. No
       harm is done if a closed leaf is derived from, and it's
       actually easy to do by syncing with a remote in which such a
       commit is made on top of that (what was formerly) a local
       closed leaf. We don't _generally_ want it do be possible on
       interactive checkins, though. This strict approach means that
       users will have to first re-open the branch (remove the
       'closed' tag) to be able to commit, which is not great UX.

       TODO, then, is to add a fsl_checkin_opt::iAmChuckNorris flag
       which decides the policy here.
    */
  ...
  if(!f->cache.markPrivate
     && (opt->isPrivate || fsl_content_is_private(f, vid))){
    f->cache.markPrivate = true;
  }
...
#define RC if(rc) goto end
  ...
  rc = fsl__checkin_calc_manifest(f, d, vid, opt);
  RC;
  if(!d->F.used){
  ...
  rc = fsl_deck_save( d, opt->isPrivate || f->cache.markPrivate );
  RC;
  assert(d->rid>0);
  ...
  rc = fsl_db_exec_multi(dbC,
                         ...
                         vid, d->rid);
  if( rc ){
    fsl_cx_uplift_db_error2(f, dbC, rc);
    goto end;
  }
  if(!rc) rc = fsl__ckout_version_write(f, d->rid, NULL);
  RC;
  assert(d->f == f);
  rc = fsl__unsent_rid_add(f, d->rid);
  RC;
  ...
  */
  ...
  //fsl__id_bag_dump(&f->cache.toVerify, NULL, "checkin pre-end");
...
#undef RC
  ...
  if(inTrans){
    if(rc) fsl_cx_txn_end_v2(f, false, false);
    else{
#if 1
      rc = fsl_cx_txn_end_v2(f, !opt->dryRun, false);
#else
...
#if 1
        if( opt->dryRun ){
          /* We need to roll back manifest file changes to avoid
             potential major confusion later. Ignore errors in this
             step, as failure to update this file is (A) outstandingly
             rare and (B) not relevant to the SCM's workings. This has
             to come _after_ the rollback and it won't work unless the
             checkout ID is first reverted back to its pre-call
             state.

             f-ci.c does something similar (or did before this
             function started doing it).
          */
          fsl__ckout_version_write(f, vid, NULL);
