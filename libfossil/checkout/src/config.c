...
static int fsl__config_notfound(fsl_cx * const f, int rc, char const *key){
  if( FSL_RC_NOT_FOUND==rc ){
    rc = fsl_cx_err_set(f, rc, "Config entry not found: %s", key);
  }
  return rc;
}
...
int fsl_config_get_buffer( fsl_cx * const f, fsl_confdb_e mode,
  ...
  return fsl__config_notfound(f, rc, key);
}
...
*/
static int fsl__config_set_prepare( fsl_cx * const f, fsl_stmt **st,
...
*/
static int fsl__config_set_versionable( fsl_cx * const f, char const * key,
                                       char const * val,
                                       fsl_size_t valLen){
  assert(key && *key);
  if(!fsl_needs_ckout(f)){
...
int fsl_config_set_text( fsl_cx * const f, fsl_confdb_e mode,
  ...
  if(!key) return FSL_RC_MISUSE;
    ...
    return fsl__config_set_versionable(f, key, val,
                                      val ? fsl_strlen(val) : 0);
  }
  fsl_db * db = fsl_config_for_role(f,mode);
  if(!db) return FSL_RC_MISUSE;
  fsl_stmt * st = NULL;
  int rc = fsl__config_set_prepare(f, &st, mode, key);
...
int fsl_config_set_int32( fsl_cx * const f, fsl_confdb_e mode,
  ...
  if(!key) return FSL_RC_MISUSE;
    ...
    return fsl__config_set_versionable(f, key, buf,
                                      fsl_strlen(buf));
  }
  fsl_db * db = fsl_config_for_role(f,mode);
  if(!db) return FSL_RC_MISUSE;
...
int fsl_config_transaction_begin(fsl_cx * const f, fsl_confdb_e mode){
  ...
  if(!db) return FSL_RC_MISUSE;
    ...
    int const rc = fsl_db_txn_begin(db);
...
int fsl_config_transaction_end(fsl_cx * const f, fsl_confdb_e mode, bool rollback){
  ...
  if(!db) return FSL_RC_MISUSE;
    ...
    int const rc = fsl_db_txn_end(db, rollback);
    if(rc) fsl_cx_uplift_db_error(f, db);
    return rc;
  }
}
...
bool fsl_config_has_versionable( fsl_cx * const f, char const * key ){
  if(!f || !key || !*key || !f->db.ckout.dir) return 0;
