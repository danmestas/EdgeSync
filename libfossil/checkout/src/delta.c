...
*/
typedef struct fsl__delta_hash fsl__delta_hash;
struct fsl__delta_hash {
...
*/
static void fsl__delta_hash_init(fsl__delta_hash *pHash,
...
*/
static void fsl__delta_hash_next(fsl__delta_hash *pHash, int c){
  const uint16_t old = pHash->z[pHash->i];
  pHash->z[pHash->i] = c;
...
*/
static uint32_t fsl__delta_hash_32bit(fsl__delta_hash *pHash){
...
/**
   ...
   fsl__delta_hash h;
   fsl__delta_hash_init(&h, zInput);
   return fsl__delta_hash_32bit(&h);
*/
static uint32_t fsl__delta_hash_once(unsigned const char *z){
...
*/
static unsigned int fsl__delta_int_put(uint32_t v, unsigned char **pz){
...
*/
static fsl_size_t fsl__delta_int_get(unsigned char const **pz, fsl_int_t *pLen){
...
*/
static int fsl__delta_digit_count(fsl_int_t v){
  unsigned int x;
  int i;
...
*/
static unsigned int fsl__delta_checksum(unsigned const char *zIn, fsl_size_t N){
  ...
  if( N>0 ){
    ...
    // ^^^ original impl, but -pedantic -Wextra warns about UB use of
    // NULL as an operand to the minus op.
    assert( ((intptr_t)z % 4)==0 );  /* Four-byte alignment */
...
#define PINT(I) intBuf = theBuf; olen=fsl__delta_int_put(I, &intBuf); OUT(theBuf,olen)
  PINT(lenOut);
  OUTCH("\n");
  ...
  */
  if( lenSrc<=NHASH ){
    ...
    OUTCH(":");
    OUT(zOut,lenOut);
    PINT((fsl__delta_checksum(zOut, lenOut)));
    OUTCH(";");
    return 0;
  }
  ...
  */
  ...
  for(i=0; i<lenSrc-NHASH; i+=NHASH){
    uint32_t const hv = fsl__delta_hash_once(&zSrc[i]) % nHash;
  ...
  */
  ...
  while( base+NHASH<lenOut ){
    ...
    fsl__delta_hash_init(&h, &zOut[base]);
    ...
    while( 1 ){
      ...
      hv = fsl__delta_hash_32bit(&h) % nHash;
      ...
      while( iBlock>=0 && (limit--)>0 ){
        ...
        */
        fsl_int_t cnt = 0, ofst = 0, litsz = 0;
        fsl_int_t j = 0, k = 0, x = 0, y = 0;
        fsl_int_t sz = 0;
        fsl_int_t limitX = 0;
        ...
        sz = fsl__delta_digit_count(i-k)
          +fsl__delta_digit_count(cnt)
          +fsl__delta_digit_count(ofst)
          +3;
      ...
      */
      ...
      fsl__delta_hash_next(&h, zOut[base+i+NHASH]);
      i++;
    }
  }
  fsl_free(collide);
 ...
 */
static int fsl__output_f_ostring( void * state, void const * src,
                                 fsl_size_t n ){
...
int fsl_delta_applied_size(unsigned char const *zDelta, fsl_size_t lenDelta_,
  ...
  if(!zDelta || (lenDelta_<2) || !deltaSize) return FSL_RC_MISUSE;
    ...
    fsl_size_t size = fsl__delta_int_get(&zDelta, &lenDelta);
    if( *zDelta!='\n' ){
...
int fsl_delta_apply2(
  ...
  */
  ...
  limit = fsl__delta_int_get(&zDelta, &lenDelta);
  if( *zDelta!='\n' ){
    if(pErr){
      fsl_error_set(pErr,
  ...
  while( *zDelta && lenDelta>0 ){
    fsl_int_t ofst = 0,
      cnt = fsl__delta_int_get(&zDelta, &lenDelta);
    switch( zDelta[0] ){
      case '@': {
        zDelta++; lenDelta--;
        ofst = fsl__delta_int_get(&zDelta, &lenDelta);
        ...
        if( (uint64_t)ofst+(uint64_t)cnt > (uint64_t)lenSrc ){
          if(pErr){
            fsl_error_set(pErr, FSL_RC_RANGE,
      ...
      case ';': {
        ...
        zOut[0] = 0;
#if !defined(FSL_OMIT_DELTA_CKSUM_TEST)
        if( cnt!=fsl__delta_checksum(zOrigOut, total) ){
          if(pErr){
