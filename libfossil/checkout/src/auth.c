...
/* All for/around getpass(). Yes, it's deprecated. */
#include <unistd.h>
#include <stdlib.h> /* free() */
#include <errno.h>

char * fsl_sha1_shared_secret2( fsl_cx * f,
                                char const * zLoginName,
                                char const * zPw,
                                char const * zProjCode ){
  fsl_sha1_cx hash;
  unsigned char zResult[20];
  char zDigest[41];

  if( !zProjCode ){
    fsl_repo_project_code(f, &zProjCode);
    if( !zProjCode ){
      /*
      ...
      */
      /* libfossil, OTOH, will need to handle that case at a different
         level, as this routine doesn't have enough info to make that
         determination. */
      return 0 /* noting that f->error has FSL_RC_NOT_FOUND or
                  FSL_RC_OOM */;
    }
  }
  ...
  assert( zProjCode );
  fsl_sha1_init(&hash);
  fsl_sha1_update(&hash, zProjCode, fsl_strlen(zProjCode));
...
char * fsl_sha1_shared_secret( fsl_cx * f,
                               char const * zLoginName,
                               char const * zPw ){
  return fsl_sha1_shared_secret2(f, zLoginName, zPw, NULL);
}

char * fsl_repo_login_group_name(fsl_cx * const f){
  return f
...
char * fsl_repo_login_cookie_name(fsl_cx * const f){
  fsl_db * db;
...
int fsl_repo_login_search_uid(fsl_cx * const f, char const * zUsername,
                              char const * zPasswd,
                              fsl_id_t * userId){
  int rc;
  char * zSecret;
...
int fsl_repo_login_clear( fsl_cx * const f, fsl_id_t userId ){
  fsl_db * db;
...
#if 0
/* TODO: (A) lock behind a feature macro and (B) port of fossil(1)'s
   getpass() alternative. */
int fsl_pw_f_getpass(fsl_cx * f, char const * zUrl,
                     fsl_buffer *b, void * state){
  char * z = 0;

  assert( 0==b->used);
  fsl_buffer_appendf(b, "%.*c\nPassword for [%s]: ",
                     (int)fsl_strlen(zUrl) + 16,
                     '*', zUrl);
  if( 0==b->errCode ){
    z = getpass( fsl_buffer_cstr(b) );
    if( z ){
      fsl_buffer_reuse(b);
      if( *z ) fsl_buffer_append(b, z, -1);
      free(z);
    }else{
      b->errCode = fsl_errno_to_rc(errno, FSL_RC_OOM);
    }
  }
  return b->errCode;
}
