...
#include "util.h" /* MUST come first b/c of config macros */
...
/**
   The length, in bytes, of a hex-format SHA1 hash string.
*/
FSL_STRLEN_SHA1 = 40,
/**
   The length, in bytes, of a hex-format SHA3-256 hash string.
*/
FSL_STRLEN_K256 = 64,
/**
   The length, in bytes, of a hex-format MD5 hash string.
*/
FSL_STRLEN_MD5 = 32,

/** Minimum length of a full fossil UUID. */
FSL_UUID_STRLEN_MIN = FSL_STRLEN_SHA1,
/** Maximum length of a full fossil UUID. */
FSL_UUID_STRLEN_MAX = FSL_STRLEN_K256
};

/**
   Unique IDs for hash types supported by fossil.
*/
...
/**
   ...
   ```
   unsigned char digest[FSL_STRLEN_MD5/2];
...
/**
   A filled-with-defaults fsl_md5_cx instance appropriate for
   const-copy initialization. Instances must either be
...
/**
   Equivalent to fsl_md5_cx_empty_m but suitable for non-const
   copy initialization.
*/
...
/**
   ...
   Alternately, copy-constructing from fsl_md5_cx_empty has the same
   effect.
...
/**
   Updates cx's state to reflect the addition of the data specified by
   the range (buf, buf+len]. Neither cx nor buf may be NULL. This may
   be called an arbitrary number of times between fsl_md5_init() and
   fsl_md5_final().
...
/**
   ...
   @see fsl_md5_final()
*/
FSL_EXPORT void fsl_md5_digest_to_base16(unsigned char const *digest,
                                         char *zBuf);
...
/**
   ...
   fsl_sha1_cx cx;
   fsl_sha1_init(&cx); // REQUIRED, unlike its md5 counterpart
...
/**
   ...
   @see fsl_sha1_final()
*/
FSL_EXPORT void fsl_sha1_digest_to_base16(unsigned char const *digest,
                                          char *zBuf);
...
/**
   Works like fsl_sha1sum_buffer() except that it appends the hash to
   the target buffer, rather than re-using the whole target. It is
   legal for both arguments to refer to the same object.
*/
FSL_EXPORT int fsl_sha1sum_buffer2(fsl_buffer const *pIn, fsl_buffer *pCksum);
...
/**
   Reads all input from src() and passes it through fsl_sha1_update(cx,...).
   Returns 0 on success, FSL_RC_MISUSE if !cx or !src. If src returns
   a non-0 code, that code is returned from here.
*/
FSL_EXPORT int fsl_sha1sum_update_stream(fsl_sha1_cx *ctx, fsl_input_f src,
                                         void * srcState);
...
/**
   ...
   of the digest. fsl_sha3_update() may be called an arbitrary number
   of times to feed in chunks of memory (e.g. to stream in arbitrarily
   large data).
*/
struct fsl_sha3_cx {
    union {
...
/**
   ...
   @see fsl_sha3_end().
*/
FSL_EXPORT void fsl_sha3_digest_to_base16(unsigned char const *digest,
                                          char *zBuf);
/**
...
*/
FSL_EXPORT int fsl_sha3sum_stream(fsl_input_f src, void * srcState,
                                  fsl_buffer *pCksum);
/**
...
*/
FSL_EXPORT fsl_hash_types_e fsl_validate_hash(const char *zHash,
                                              fsl_size_t nHash);
 ...
 */
FSL_EXPORT const char * fsl_hash_type_name(fsl_hash_types_e h,
                                           const char *zUnknown);
