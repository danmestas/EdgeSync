...
/** @page page_fcli fcli (formerly FossilApp)
    ...
    - fcli_setup_v2() bootstraps the environment. This must be the
...
/**
   Outputs the given fsl_appendf()-compatible formatting expression
   (2nd arg) to fcli_printf() if fcli.verbose>=N, else it does
   nothing.
...
/**
   Models a single CLI flag. Its relative complexity is largely to
   support generation of help text.
...
/** @def FCLI_FLAG_xxx
   ...
   int rc = fcli_setup_v2(argc, argv, cliFlags, &appHelp);
   ```
...
/** int64-type flag with a callback. */
...
struct fcli_t {
  ...
  */
  ...
  /**
     ...
     However, some of this state is only be legally modified
     _before_ calling fcli_setup() (or equivalent).
  */
  struct {
    /**
    ...
    */
    int traceSql;
    /**
       This output channel is used when initializing the context
       object. The default implementation uses fsl_outputer_FILE to
       output to stdout.
    */
    fsl_outputer outputer;

    /**
       Listener for fsl_msg-published messages.
    */
  ...
  struct {
    ...
    */
    ...
    /**
       ...
       This is an int, rather than a bool, to support multiple levels
       of help verbosity via multiple --help flags.
    */
    short helpRequested;
    /**
  ...
  */
  ...
  struct {
    /**
       Key passed to the --see-key CLI flag.
    */
    const char * key;
...
/**
   ...
   `fcli.config.checkoutDir` to `NULL` before calling this.

   If the 3rd argument is not NULL, this sets `fcli.config.cliFlags` to its
   value. If the 4th argument is not NULL, this sets `fcli.config.appHelp` to
   that value.
   ...
   is not NULL or if fcli.config.cliFlags has been set before it is
...
/**
   ...
   *value gets assigned to a COPY OF the value part of the first form
   ...
   This function may update the fcli error state. Specifically, if
...
/**
   ...
   If fcli_cx() has no error state but fsl_cx_db(fcli_cx()) does,
   then that error state is moved into the fsl_cx object and
   reported here.
...
/**
   ...
   If fcli.config.cliFlags is set before fcli_setup() is called, this routine
...
/**
   ...
   If fcli.config.cliFlags is set before fcli_setup() is called, this
   routine is called and passed those flags. Thus most apps can simply
   assign their flags there and let fcli_setup() do the work. Apps
   which have multiple dispatch paths, each with differing flags, may
   find it easier to use this.

   Such apps, in order to get the full help listing for all dispatch
   paths, may need to assign fcli.config.appHelp to a helper which
...
/**
   ...
   If the 2nd argument is true AND the cmd->usage() callback has been
   set, it will be called for each object in the cmd array after
   outputting the cmd->flags help text.
...
/**
   A fsl_msg_f() implementation intended for use with CLI apps.  It
   emits all output via fcli_printf(). This is the default message
   listener. Clients may replace the default by setting up
   fcli.config.listener before calling fcli_setup_v2(), and simple
   client apps may use this for other fsl_cx instances. This
   implementation ignores its state argument.
*/
FSL_EXPORT int fsl_msg_f_fcli(fsl_msg const *msg, void *state);
