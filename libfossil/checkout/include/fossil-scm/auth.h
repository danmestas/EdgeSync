...
/**
   ...
   "zProjectCode/zLoginName/zPw"

   (without the quotes)

   where zProjectCode is typically a repository-dependent series of
   random bytes. If zProjCode is 0 then fsl_repo_project_code() is
   used to fetch it. (The project code is stored in the repository's
   config table under the key 'project-code'.)

   The returned string is owned by the caller, who must eventually
   fsl_free() it.

   All arguments must be non-NULL or behavior is undefined.
...
FSL_EXPORT char * fsl_sha1_shared_secret2( fsl_cx * const f, char const * zLoginName,
                                           char const * zPw, char const * zProjCode );

/**
   Equivalent to fsl_sha1_shared_secret2(f,zLoginName,zPw,NULL).
*/
FSL_EXPORT char * fsl_sha1_shared_secret( fsl_cx * const f, char const * zLoginName,
                                          char const * zPw );
