...
/**
  Copyright 2013-2025 Stephan Beal (https://wanderinghorse.net).
...
/*
   libfossil.h MUST be included first so we can set some portability
...
namespace fsl {
  ...
  */
  class Context{
    ...
    */
    ...
    /**
       ...
       Throws on error.
    */
    explicit Context(fsl_cx_config const & opt);
    ...
    */
    ...
    /**
       ...
       pointer ownership may get confused. It will appear to work at
       times, but certain combinations of operations via the C API
       (e.g. opening a checkout, closing it, then opening a standalone
       repo) might get some C++-side pointers cross-wired
       (theoretically/hypothetically).

       - ... as an argument to a function which takes a void pointer.
    */
    ...
    */
    ...
    void setup(fsl_cx_config const * opt);
  };
  ...
  /**
     ...
     fsl_deck::F.list will sometimes, but not always, behave much
  ...
  public:
    ...
    */
    ...
    /**
       ...
       Reminder: traversal of F-cards does not include deleted files.
       Fossil's base manifest format does not record deletions (files
       are deleted by virtue of not being in the manifest).  Delta
       manifests record them but they are not traversed because of the
       inconsistency and extra work that difference would require in
       many algorithms which traverse F-cards.
  ...
  */
  ...
  private:
    ...
    /**
       ...
       proxy function.
    */
    static int fslAppendfF( void * state, void const * s, fsl_size_t n );
