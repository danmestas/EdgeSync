...
namespace fsl {
  ...
  void Deck::propagateError() const{
    fsl_error const * err = fsl_cx_err_ec(this->cx);
    if(err->code){
      throw Exception(err);
    }
  }
  ...
  Deck const & Deck::assertHasRequiredCards() const{
    if(!fsl_deck_has_required_cards(*this)){
      throw Exception(fsl_cx_err_ec(this->cx));
    }
    return *this;
  }
  ...
  Deck const & Deck::output( fsl_output_f f, void * outState ) const{
    ...
    if(rc) throw Exception(fsl_cx_err_ec(this->d->f));
    return *this;
  }
  ...
  Deck::FCardIterator::FCardIterator(Deck & d)
    : d(&d),
      fc(NULL)
  {
