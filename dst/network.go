...
// It supports message dropping and network partitions,
...
// Transport returns a libfossil.Transport for the given node that routes
...
func (n *SimNetwork) exchange(ctx context.Context, nodeID NodeID, payload []byte) ([]byte, error) {
	...
	return n.bridge.HandleRequest(ctx, payload)
}

// SimTransport implements libfossil.Transport by routing through the SimNetwork.
type SimTransport struct {
	network *SimNetwork
	nodeID  NodeID
}

// RoundTrip sends a request through the simulated network to the bridge.
func (t *SimTransport) RoundTrip(ctx context.Context, payload []byte) ([]byte, error) {
	return t.network.exchange(ctx, t.nodeID, payload)
}
