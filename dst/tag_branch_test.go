...
import (
	...
	"github.com/dmestas/edgesync/go-libfossil/branch"
...
// This would catch bugs in tag propagation, Crosslink T-card processing,
// or unsent marking of control artifacts.
func TestTagPropagationAcrossSync(t *testing.T) {
	masterRepo := createMasterRepo(t)
	mf := NewMockFossil(masterRepo)

	// Create checkins via manifest.Checkin (not blob.Store).
	// The initial checkin adds inline T-cards: *branch trunk, +sym-trunk.
