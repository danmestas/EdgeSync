...
// Stat returns insertion/deletion/change counts between a and b.
...
type DiffStat struct {
    ...
    Changes    int // lines present in both but different content
}
```
...
The Myers algorithm works on the "edit graph" — finding the shortest path from (0,0) to (n,m) where horizontal moves are deletions, vertical moves are insertions, and diagonal moves are equal lines. Uses the classic "snakes" approach with O(nd) time and O(n) space (linear space refinement).
...
Handles line truncation when lines exceed column width. Uses `|` for equal, `<` for delete-only, `>` for insert-only, and `!` for changed lines (present on both sides but different).
...
**Golden tests:**
- Compare unified output against `fossil diff` on same inputs (fixture files)
- Compare side-by-side output against `fossil diff -y` on same inputs
...
- Binary files: detect null bytes, return empty diff
- `splitLines` must handle `\r\n` (strip `\r` before comparison, consistent with Fossil)
